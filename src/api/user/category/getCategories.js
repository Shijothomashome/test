...
export const getCategoriesAndSubCategories = async (req, res, next) => {
    try {
        ...
        if (type === "selected" && (!parent_id || !mongoose.Types.ObjectId.isValid(parent_id))) {
            throw new BadRequestError("Invalid or missing category ID");
        }

        const parentCategories = await categoryModel.aggregate([{$match:{ parentCategory: null }},{$project:{name:1}}]);
